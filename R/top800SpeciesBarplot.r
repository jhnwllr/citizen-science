...
p = ggplot(PD, aes(specieskeys,occCounts, fill = plotCat)) + 
  ...
  ylab("number of occurrences (millions)") + 
  xlab("top 800 species") + 
  geom_label(aes(y = (occCounts + 1e6),label=barLabels),label.padding = unit(0.1, "lines"),hjust=0,fill = "white",alpha=0.5, position=position_dodge(width=1), size=3) +
  geom_label(aes(y = occCounts + 0.91e6,label=barLabelsBoxesBirds),label.padding = unit(0.1, "lines"),label.r = unit(0.1, "lines"),hjust=0,fill = "#dddddd", position=position_dodge(width=1), size=3) +
  geom_label(aes(y = occCounts + 0.91e6,label=barLabelsBoxesMammals),label.padding = unit(0.1, "lines"),label.r = unit(0.1, "lines"),hjust=0,fill = "#D66F27", position=position_dodge(width=1), size=3) +
  geom_label(aes(y = occCounts + 0.91e6,label=barLabelsBoxesPlants),label.padding = unit(0.1, "lines"),label.r = unit(0.1, "lines"),hjust=0,fill = "#509E2F", position=position_dodge(width=1), size=3) +
  geom_label(aes(y = occCounts + 0.91e6,label=barLabelsBoxesInsects),label.padding = unit(0.1, "lines"),label.r = unit(0.1, "lines"),hjust=0,fill = "#777777", position=position_dodge(width=1), size=3) + 
  geom_errorbar(aes(ymax=ymax, ymin=ymin),data = barLines,color= "#777777",width=0.001,size=0.05,stat="identity")

saveFileName = "C:/Users/ftw712/Desktop/citizen science/plots/top800speciesBarplotWeb"

ggsave(saveFileName %+% ".pdf", plot = p, device=cairo_pdf,width=9,height=9)
ggsave(saveFileName %+% ".jpg", plot = p,width=9,height=7,units="in",dpi=900)
ggsave(saveFileName %+% ".tiff", plot = p,width=9,height=7,units="in",dpi=600)
ggsave(saveFileName %+% ".svg", plot = p,width=9,height=7)

# save one for the blog
ggsave("C:/Users/ftw712/Desktop/data-blog/static/post/2018-12-10-gbif-citizen-science-data_files/top800speciesBarplotWeb.jpg",width=9,height=9,units="in",dpi=900)
